...
class FireSpreadDataset(Dataset):
    def __init__(self, data_dir: str, included_fire_years: List[int], n_leading_observations: int,
                 ...
                 features_to_keep: Optional[List[int]] = None, return_doy: bool = False, predict_active_map_only: bool = True):
        """_summary_
        ...
        In practice, this means that if n_leading_observations is smaller than this value, some samples are skipped. Defaults to None. If None, nothing is skipped. This is especially used for the train and val set. 
            ...
            predict_active_map_only (bool, optional): _description_. If True, the output is the binary active fire mask only. If False, the output is the full image. Defaults to True.
        ...
        self.predict_active_map_only = predict_active_map_only
    ...
    def load_imgs(self, found_fire_year, found_fire_name, in_fire_index):
        ...
        #     if self.predict_active_map_only:
        #         y = y[0, -1, ...]
        # else:
    ...
    def preprocess_and_augment(self, x, y):
        ...
        if not self.load_from_hdf5:
            ...
            x[:, -1, ...] = torch.div(x[:, -1, ...], 100, rounding_mode='trunc')
        ...
        if self.predict_active_map_only:
            ...
            # if we predict the full image, do the same for the target
            y[:, self.indices_of_degree_features, ...] = torch.sin(
                torch.deg2rad(y[:, self.indices_of_degree_features, ...]))

            binary_af_mask = (y[:, -1:, ...] > 0).float()

            y = self.standardize_features(y)

            y = torch.cat([y, binary_af_mask], axis=1)

            y = torch.nan_to_num(y, nan=0.0)

            new_shape = (y.shape[0], y.shape[2], y.shape[3],
                         self.one_hot_matrix.shape[0])

            landcover_classes_flattened = y[:, 16, ...].long().flatten() - 1
            landcover_encoding = self.one_hot_matrix[landcover_classes_flattened].reshape(
                new_shape).permute(0, 3, 1, 2)

            y = torch.cat(
                [y[:, :16, ...], landcover_encoding, y[:, 17:, ...]], dim=1)
    ...
    def augment(self, x, y):
        """_summary_ Applies geometric transformations: 
          1. random square cropping to 256x256, preferring images with a) fire pixels in the output and b) (with much less weight) fire pixels in the input
        ...
        Returns:
            _type_): _description_ Augmented tensors of size 256x256
        """
        ...
        for i in range(10):
            top = np.random.randint(0, x.shape[-2] - target_size)
            left = np.random.randint(0, x.shape[-1] - target_size)
            x_crop = TF.crop(x, top, left, target_size, target_size)
            y_crop = TF.crop(y, top, left, target_size, target_size)
            ...
            if self.predict_active_map_only:
                ...
                n_fire_pixels = x_crop[:, -1, ...].mean() + 1000 * y_crop[:, -1, ...].mean()
    ...
    def center_crop_x32(self, x, y):
        """_summary_ Crops the center of the image to 256x256, which matches the pretrained model size.
        Only used for computing the test performance.

        Args:
            x (_type_): _description_ Input tensor of shape (T, C, H, W)
            y (_type_): _description_ Target tensor of shape (H, W) or (C, H, W)

        Returns:
            _type_): _description_ Cropped tensors of size 256x256
        """
        T, C, H, W = x.shape
        # Calculate crop size to get 256x256
